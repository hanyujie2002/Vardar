...
export const handleNode = (config) => (node) => {
  ...
  if (config.titleLift) {
    ...
    const paragraphModified = paragraph;
    if (
      paragraphModified.children.length > 0 &&
      paragraphModified.children[0].type == 'text'
    ) {
      ...
      text.value =
        config.titleLiftWhitespaces(whitespaces) +
        text.value.slice(whitespaces.length);
    }
  }
};
